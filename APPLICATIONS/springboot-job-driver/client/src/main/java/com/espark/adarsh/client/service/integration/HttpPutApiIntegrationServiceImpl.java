...
@Slf4j
@Service
public class HttpPutApiIntegrationServiceImpl<T> implements ApiIntegrationService<T> {

    private final ApiRestTemplateFactory apiRestTemplateFactory;

    public HttpPutApiIntegrationServiceImpl(ApiRestTemplateFactory apiRestTemplateFactory) {
        this.apiRestTemplateFactory = apiRestTemplateFactory;
    }

    @Retry(name = Constants.PUT_API_RETRY)
    @CircuitBreaker(name = Constants.PUT_API_CIRCUIT_BREAKER, fallbackMethod = "executeCircuitBreakerPut")
    @RateLimiter(name = Constants.PUT_API_RATE_LIMITER, fallbackMethod = "executeRateLimiterFallbackPut")
    public ApiResponse<T> putApiCallExecution(AbstractApiDetails apiDetails, String jobName) {
        try {
           ...
           return errorResponse.apply(statusResponseEntity.getBody(),"HttpStatusCode from Server "+statusResponseEntity.getStatusCode());
            ...
            log.error("PutApiCallExecution Retryable {}", e.getMessage());
            throw e;
        } catch (HttpClientErrorException | HttpServerErrorException e) {
            log.error("PutApiCallExecution Non-Retryable {}", e.getMessage());
            return errorResponse.apply(null, e.getMessage());
        } catch (Exception e) {
            log.error("PutApiCallExecution Exception Occurred {}", e.getMessage());
            return errorResponse.apply(null, e.getMessage());
        }
    }

    public ApiResponse<T> executeCircuitBreakerPut(CallNotPermittedException callNotPermittedException) {
        log.error("HttpPutApiIntegrationServiceImpl:: executeCircuitBreakerGet {}", callNotPermittedException.getMessage());
        return errorResponse.apply(null, callNotPermittedException.getMessage());
    }

    public ApiResponse<T> executeRateLimiterFallbackPut(RequestNotPermitted requestNotPermitted) {
        log.error("HttpPutApiIntegrationServiceImpl:: executeRateLimiterFallbackGet {}", requestNotPermitted.getMessage());
