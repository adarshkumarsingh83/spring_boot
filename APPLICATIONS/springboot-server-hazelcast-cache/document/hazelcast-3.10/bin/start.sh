...
export CLASSPATH="$HAZELCAST_HOME/lib/hazelcast-all-3.10.jar"
...
if [ -z "${PID}" ]; then
    ...
    echo "Another Hazelcast instance (PID=${PID}) is already started in this folder. To start a new instance, please unzip hazelcast-3.10.zip/tar.gz in a new folder."
    exit 0
fi
